...
func (Tenant) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("name").MaxLen(50).Unique().SchemaType(map[string]string{
		...
		field.String("brand_name").MaxLen(50).Optional().Unique().SchemaType(map[string]string{
			dialect.Postgres: "varchar(50)",
		}),
		field.String("domain").MaxLen(200).Optional().Unique().SchemaType(map[string]string{
			dialect.Postgres: "varchar(200)",
		}),
	}
}
